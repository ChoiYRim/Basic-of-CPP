...
public:
    card(int n,suit _s) : number(n), s(_s) { }
...
private:
    ...
    {
        for(int i = 0; i < 13; i++)
            deck[i] = card{i+1,HEART};
        for(int i = 0; i < 13; i++)
            deck[i+13] = card{i+1,SPADE};
        for(int i = 0; i < 13; i++)
            deck[i+26] = card{i+1,CLUB};
        for(int i = 0; i < 13; i++)
            deck[i+39] = card{i+1,DIAMOND};
    }
    ...
    {
        if(p1.back().get_number() == p2.back().get_number())
        {
            p1.pop_back();
            p2.pop_back();
            return true;
    ...
    void playOneRound()
    {
        ...
        auto seed = std::chrono::system_clock::now().time_since_epoch().count();
        std::shuffle(player1.begin(),player1.end(),std::default_random_engine(static_cast<unsigned int>(seed)));
        std::shuffle(player2.begin(),player2.end(),std::default_random_engine(static_cast<unsigned int>(seed)));
        std::shuffle(player3.begin(),player3.end(),std::default_random_engine(static_cast<unsigned int>(seed)));
        std::shuffle(player4.begin(),player4.end(),std::default_random_engine(static_cast<unsigned int>(seed)));
