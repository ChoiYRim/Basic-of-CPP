...
public:
    ...
    unsigned find(unsigned x)
    {
        ...
        while(node_id != nodes[node_id].parent) // 트리의 루트를 찾을 때까지
        {
    ...
    void union_sets(unsigned x,unsigned y)
    {
        ...
        if(root_x == root_y) // 같은 트리안에 있음
            return;
        
        // 작은 rank의 트리를 큰 rank의 트리 쪽으로 병합
...
public:
    ...
    void add_edge(Edge<T>&& e)
    {
        ...
        if(e.src >= 1 && e.src <= V && e.dst >= 1 && e.dst <= V)
            ...
            cerr << "Error: out of ranged edge!" << endl;
    }
...
int main(int argc,char* argv[])
{
    ...
    for(auto& i : edge_map)
        for(auto& j : i.second)
            G.add_edge(Edge<unsigned int>{i.first,j.first,j.second});
